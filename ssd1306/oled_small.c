...
const uint8_t ssd1306_init_sequence [] = {  // Initialization Sequence
    ...
    0xA8, 0x3F,     // Set multiplex ratio (1 to 32) ... (height - 1)
    ...
    0x22, 0, 63,   // Set Page Address (start,end) 0 - 31
    0x21, 0, 127,   // Set Column Address (start,end) 31 - 95
};
...
void OLED_setline(uint8_t line) {
    ...
    I2C_write(OLED_PAGE + line);        // set page (line) – OLED_PAGE is 0xB0
    I2C_write(OLED_COLUMN_LOW);         // set lower column start to 0
    I2C_write(OLED_COLUMN_HIGH);        // set higher column start to 0
...
void OLED_clear(void) {
    ...
    I2C_start(OLED_ADDR);                           // start transmission to OLED
    I2C_write(OLED_DAT_MODE);                       // set data mode
    for(i=64*32; i; i--) {                          // clear screen and buffer
        I2C_write(0x00);
    }           
    I2C_stop();                                     // stop transmission
}
...
void OLED_plotChar(uint16_t c, bool inverted) {
    I2C_start(OLED_ADDR);                   // start transmission to OLED
    ...
    if (index <= 127) {
        ...
        for(uint8_t i=0 ; i!=5; i++){
            I2C_write(inverted? ~(font5x8[index][i]) : font5x8[index][i]);
        }
    } else {
        for(uint8_t i=0 ; i!=5; i++){
            I2C_write(inverted? ~(font5x8[94][i]) : font5x8[94][i]);
        }
    }
...
void OLED_fill(uint8_t p) {
  ...
  I2C_start(OLED_ADDR);                   // start transmission to OLED
  I2C_write(OLED_DAT_MODE);               // set data mode
  for(uint16_t i=96*4; i; i--){          // send pattern
    I2C_write(p);
  } 
  I2C_stop();                             // stop transmission
}
